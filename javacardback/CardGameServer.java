...
public class CardGameServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        whiteCards.add("Поедание бороды Перельмана, чтобы получить его знания");
        ...
        whiteCards.add("Падение с кровати");
        ...
        whiteCards.add("Батины подштанники");
        whiteCards.add("Пони-страпони");
        whiteCards.add("Дрожжи");
        whiteCards.add("Черные люди");
        whiteCards.add("Двойное проникновение");
        whiteCards.add("Ультранасилие");
        whiteCards.add("Двойное проникновение");
        ...
        redCards.add("На седьмом круге ада грешники должны терпеть ___ целую вечность");
        redCards.add("Наука не может объяснить ___");

        redCards.add("Папа, почему мама плачет?");
        redCards.add("Хорошее собеседование начинается с рукопожатия и оканчивается ___");
        redCards.add("Что родители скрывают от меня?");
        redCards.add("Секрет идеального брака — это ___");
        redCards.add("Когда бьюти-блогеры вырастают, они становятся ___");
        redCards.add("Любовь приходит и уходит, а ___ хочется всегда");
        redCards.add("___ мешает людям вести ЗОЖ");
        redCards.add("Мне нужны наркотики, чтобы упороться, ведь я упарываюсь ___");
        ...
        server.createContext("/api/replace", new ReplaceHandler());
    ...
    static class StateHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            state.put("currentRound", gameState.currentRound);
            state.put("whiteCardCount", whiteCards.size());
            state.put("redCardCount", redCards.size());
    ...
    static class ReplaceHandler implements HttpHandler {
            @Override
            public void handle(HttpExchange exchange) throws IOException {
                if ("OPTIONS".equals(exchange.getRequestMethod())) {
                    sendResponse(exchange, 200, "");
                ...
                String[] parts = new Scanner(exchange.getRequestBody()).nextLine().split(":");
                String playerName = parts[0];
                String cardToReplace = parts[1];

                Player player = players.get(playerName);

                whiteCards.remove(cardToReplace);
                player.cards.remove(cardToReplace);

                String newCard = whiteCards.remove(0);
